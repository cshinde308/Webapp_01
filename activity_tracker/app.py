...
from functions import init_session,get_year_sheet
...
if page == "Activity Tracker":
    ...
    with tab_project_dashboard:
        ...
        # -------------------------------------------------
        # AUTO-DETECT YEARS FROM DATE RANGE
        # -------------------------------------------------
        start_year = start_date.year
        end_year = end_date.year

        # -------------------------------------------------
        # LOAD DATA FROM ALL RELEVANT YEAR SHEETS
        # -------------------------------------------------
    ...
    with tab_log:
        ...
        if st.button("✅ Submit", key="log_submit_btn"):
            ...
            activity_year = base_date.year
            ws = get_year_sheet(wb, activity_year)
    ...
    with tab_editactivity:
        ...
        if st.session_state.searched:
            ...
            activity_year = st.session_state.search_date.year
            if str(activity_year) not in wb.sheetnames:
                wb.close()
                st.session_state.preview_df = None
                st.warning("No activities found for selected year.")
            else:
                ws = wb[str(activity_year)]
