...
def get_activity_block_html(target_date,excel_file):
    ...
    if str(target_year) not in wb.sheetnames:
        return "<i>No activity found</i>"
    ws = wb[str(target_year)]
...
HEADERS = ["Date", "Time", "Project", "Engineer", "Hours", "Note"]

def get_year_sheet(wb, year: int):
    sheet_name = str(year)
