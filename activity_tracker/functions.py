...
def render_preview_with_multiline_notes2(df, height_px=400):
    """
    Render dataframe as scrollable HTML table
    with proper multiline handling.
    """
    df = df.copy()
    ...
    for col in df.columns:
        if df[col].dtype == object:
            df[col] = (
                ...
                .astype(str)
                .str.replace(r"\\n", "<br>", regex=True)     # typed \n
                .str.replace(r"\r?\n", "<br>", regex=True)  # real newline
                .apply(lambda x: f"<div style='white-space:pre-wrap'>{x}</div>")
            )

    html = f"""
    <div style="max-height:{height_px}px;overflow:auto;border:1px solid #ccc;">
